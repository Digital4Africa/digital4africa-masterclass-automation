...
const StatsCards = ({ masterclasses }) => {
  ...
  const totalStudents = masterclasses.reduce((sum, m) => sum + m.enrolled, 0);
  const totalRevenue = masterclasses.reduce((sum, m) => sum + (m.paidStudents * m.price), 0);
  ...
  const stats = [
    ...
    { title: 'Total Students', value: totalStudents, change: '+12 from last month', icon: '👥' },
    { title: 'Total Revenue', value: `$${totalRevenue}`, change: '+15% from last month', icon: '💰' },
  ];
